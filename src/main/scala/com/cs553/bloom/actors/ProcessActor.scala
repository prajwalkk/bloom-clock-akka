...
object ProcessActor extends LazyLogging {
  ...
  val probabilities: Map[Double, String] = TreeMap(0.0 -> "None", 0.1 -> "Send", 0.9 -> "Internal")
  val probabilitiesNormalized: Map[Double, String] = probabilities.map { case (k, v) =>
    k / probabilities.keys.sum -> v
  }
  ...
  def apply(numProcesses: Int,
    ...
    Behaviors.setup { context: ActorContext[ProcessMessages] =>
      ...
      new ProcessActor(context, guardRef, writerRef, numProcesses, K, M)
        .behaviorsOfProcess(processID, bloomClock, vectorClock, 0, None)
    }
...
class ProcessActor(context: ActorContext[ProcessActor.ProcessMessages],
  ...
  def behaviorsOfProcess(processID: Int,
                         bloomClock: List[Int],
    ...
    Behaviors.receiveMessagePartial {
      ...
      case ListingResponse(ProcessKey.Listing(listings)) =>
        context.log.debug(listings.toString())
        context.log.debug((listings - context.self).toString())
        behaviorsOfProcess(processID, bloomClock, vectorClock, eventCounter, Some(listings - context.self))


      case ExecuteSomething =>
        if (processRefs.getOrElse(0) == 0) {
          Behaviors.ignore
      ...
      case RecvMessage(senderVectorClock, senderBloomCLock) =>
        ...
        behaviorsOfProcess(processID, bloomClock, vectorClock, eventCounter, processRefs)
      ...
      case ReceivedSendGSN(gsnValue, whom) =>
        ...
        if (gsnValue.gsn != -1) {
          ...
          behaviorsOfProcess(processID, newBC, newVC, newEventCounter, processRefs)
        } else {
          behaviorsOfProcess(processID, bloomClock, vectorClock, eventCounter, processRefs)
        }
  ...
  def randomExec(probs: Map[Double, String]): String = {
    ...
    context.log.debug(s"$ranges, $res")
    res
  }
